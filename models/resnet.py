...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.stride = stride
        self.out_channels = self.expansion * planes
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        if self.interm_feat:
            if self.profiling != 0:
                self.forward = self._forward_prof
            else:
                self.forward = self._forward_interm
        else:
            if self.profiling != 0:
                self.forward = self._forward_prof
            else:
    ...
    def _forward_interm(self, x: Tensor) -> Tensor:
    ...
    def _forward_prof(self, x: Tensor) -> Tensor:
        interm_feat = []
        interm_feat.append(x)
        x = self.conv1(x)
        x = self.bn1(x)
    ...
    def _forward_ori(self, x: Tensor) -> Tensor:
        # See note [TorchScript super()]
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
