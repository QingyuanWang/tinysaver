...
class Block(nn.Module):
    ...
    def __init__(self, dim, upscale=4, dim_out=None, drop_path=0.):
        super().__init__()
        self.dwconv = nn.Conv2d(dim, dim, kernel_size=7, padding=3, groups=dim)  # depthwise conv
        self.norm = LayerNorm(dim, eps=1e-6)
        self.pwconv1 = nn.Linear(dim, upscale * dim)  # pointwise/1x1 convs, implemented with linear layers
        self.act = nn.GELU()
        self.grn = GRN(upscale * dim)
        self.pwconv2 = nn.Linear(upscale * dim, dim)
...
class ConvNeXtV2(nn.Module):
    ...
    def __init__(
        ...
        if no_head:
            self.forward = self.forward_features_
        else:
        ...
        self.num_layers = sum([len(s) for s in self.stages])
        self.num_classes = num_classes
        self.profiling = profiling
        if self.profiling > 0:
            self.forward = self.forward_features_profiling
    ...
    def forward_(self, x):
        x, interm_feat = self.forward_features_(x)
